...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        /*
         * switch n & m so that n is always greater than m
         */
        if (m.compareTo(n) > 0) {
            NaturalNumber tmp = n.newInstance();
            tmp.transferFrom(n);
            n.transferFrom(m);
            m.transferFrom(tmp);
        }
        /*
         * calculate gcd of n & m
         */
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber tmp = n.newInstance();
        tmp.copyFrom(n);
        NaturalNumber pow = n.newInstance();
        pow.copyFrom(p);
         ...
         */
        if (p.equals(one)) {
            NaturalNumber tmp1 = n.divide(m);
            n.copyFrom(tmp1);
        }
        /*
         * for numbers with powers more than 1
         */
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
         ...
         * (w ^ 2 mod n = 1)
         */
        NaturalNumber wit = w.newInstance();
        wit.copyFrom(w);
        powerMod(wit, two, n);
        if (wit.equals(one)) {
            witness = true;
         ...
         * (w ^ (n-1) mod n /= 1)
         */
        NaturalNumber nLess = w.newInstance();
        nLess.copyFrom(n);
        nLess.decrement();
        wit.copyFrom(w);
        powerMod(wit, nLess, n);
        if (!wit.equals(one)) {
            witness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < twentyFive; i++) {
                /*
                 * requires n>0. ensures randomNumber = [a random number
                 * uniformly distributed in [0, n]]. returns a NaturalNumber.
                 * need witness to be in [2, n – 1].
                 */
                NaturalNumber nLess = n.newInstance();
                nLess.copyFrom(n);
                nLess.decrement();
                nLess.decrement();
                NaturalNumber witness = randomNumber(nLess);
                while (witness.compareTo(two) < 0) {
                    witness = randomNumber(nLess);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
         ...
         * the odd numbers only (why?), until n is likely prime. Check only odd
         * numbers as even numbers are always composite.
         */
