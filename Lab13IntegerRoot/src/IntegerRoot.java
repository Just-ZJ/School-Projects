...
public final class IntegerRoot {
    ...
    private static int power(int n, int p) {
        int result = 1, count = 0, evenPower = n * n;
        /*
         * should take lesser calculations now
         */
        while (count < p / 2) {
            result *= evenPower;
            count++;
    ...
    private static int root(int n, int r) {
        int guess = 0, upperLim = n + 1, lowerLim = 0, check = 0;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            ...
            if (x == results[i]) {
                out.println("Test " + i + " passed: root(" + numbers[i] + ", "
                        + roots[i] + ") = " + results[i]);
            } else {
                out.println("*** Test " + i + " failed: root(" + numbers[i]
