...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combined = str1.substring(0, str1Len - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        if (!strSet.contains(str)) {
            ...
            boolean strIsSubOfEle = false;
            Set<String> removeFrom = strSet.newInstance();
            for (String element : strSet) {
                /*
                 * checks if str is a substring of element
                 */
                int eleStart = element.indexOf(str);
                if (eleStart >= 0) {
                    int strLen = str.length();
                    String sub = element.substring(eleStart, eleStart + strLen);
                    if (sub.equals(str)) {
                        strIsSubOfEle = true;
                    }
                }
                ...
                /*
                 * checks if element is a substring of str
                 */
                int strStart = str.indexOf(element);
                if (strStart >= 0) {
                    int eleLen = element.length();
                    String sub = str.substring(strStart, strStart + eleLen);
                    if (sub.equals(element)) {
                        removeFrom.add(element);
                    }
                }
            }

            /*
             * strIsSubOfEle is false. str is not substring of any element
             */
            if (!strIsSubOfEle) {
                strSet.add(str);
                /*
                 * element is a substring of str
                 */
                if (removeFrom.size() > 0) {
                    strSet.remove(removeFrom);
                }
            }
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> strInFile = new Set2<String>();
        String tmp = input.nextLine();
        strInFile.add(tmp);

        while (!input.atEOS()) {
            tmp = input.nextLine();
            addToSetAvoidingSubstrings(strInFile, tmp);
        }
        return strInFile;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        if (text.length() > 0) {
            ...
            if (text.contains(replace)) {
                int startPos = text.indexOf(replace);
                if (startPos != 0) {
                    out.print(text.substring(0, startPos));
                    out.println();
                } else {
                    out.println();
                }
                String tmp = text.substring(startPos + 1, text.length());
                printWithLineSeparators(tmp, out);
            } else {
                out.print(text);
            }
        }
