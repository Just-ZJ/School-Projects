...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            String tmp = line.substring(pos + 1, line.length());
            Integer val = 0;
            if (FormatChecker.canParseInt(tmp)) {
                val = Integer.parseInt(tmp);
            }
            priceMap.add(word, val);
        }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (!tmp.equals("") && !input.atEOS()) {
            if (sizePriceMap.hasKey(tmp)) {
                totPrice += sizePriceMap.value(tmp);
            } else if (toppingPriceMap.hasKey(tmp)) {
                totPrice += toppingPriceMap.value(tmp);
            }
            tmp = input.nextLine();
        }
        return totPrice;
