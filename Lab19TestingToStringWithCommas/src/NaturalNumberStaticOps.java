...
public final class NaturalNumberStaticOps {
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        NaturalNumber natThousand = n.newInstance();
        natThousand.setFromInt(ONE_THOUSAND);
        ...
        if (tmp.compareTo(natThousand) > 0) {
            NaturalNumber digit = tmp.divide(natThousand);
            printWithCommas(tmp, out);
            out.print("," + digit);
        } else {
            out.print(tmp);
        }
    ...
    public static String toStringWithCommas(NaturalNumber n) {
        ...
        if (tmp.compareTo(natThousand) > 0) {
            NaturalNumber digit = tmp.divide(natThousand);
            numStr = toStringWithCommas(tmp) + "," + digit;
        } else {
            numStr = tmp + numStr;
        }
        return numStr;
